...
def main(scenario):    
    ...
    for i in range(len(t)):
       ...
       # mean_interference_policy_train_sims2
    ...
    #CUSTOM ADD Slicing
    #plt.plot(t, np.array(interference_performance_policy2) / avg_result_over, label='INTERFERENCE',
  ...
  #  sizeiteration = len(sum_rate_performance_policy)
    sr1 = np.array(sum_rate_performance_policy)/avg_result_over
    sr2 = np.array(sum_rate_performance_policy2) / avg_result_over
    sr3 = np.array(sum_rate_performance_FP) / avg_result_over
    sr4 = np.array(sum_rate_performance_FPMulti_delayedbyone) / avg_result_over
    sr5 = np.array(sum_rate_performance_random) / avg_result_over


   # with open('./fig/sumrateoutput%s_network_%d'%(json_file,overal_sims)+'.csv', mode='w') as employee_file:
   #     employee_writer = csv.writer(employee_file, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
   #     for ele in range(sizeiteration):
   #         if str(sr1[ele]) != 'nan':
   #             employee_writer.writerow([t[ele], sr1[ele], sr1[ele], sr2[ele], sr3[ele], sr4[ele], sr5[ele]])

    extmatlab.savemat('./fig/sumrateoutput%s_network_%d'%(json_file,overal_sims)+'.mat', mdict={'time_sample': t, 'se_1':sr1, 'se_2':sr2, 'se_3':sr3, 'se_4':sr4, 'se_5':sr5})
