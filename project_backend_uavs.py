...
def sumrate_multi_list_clipped_perslicing(H, p, alpha, noise_var):
    # Take pow 2 of abs_H, no need to take it again and again
    H_2 = H ** 2
    N = alpha.shape[0]  # number of links
    M = alpha.shape[1]  # number of channels
    ...
    for out_loop in range(M):
        for loop in range(N):
            ...
            tmp_2 = np.matmul(np.multiply(alpha[:, out_loop], H_2[loop, :, out_loop]), p) + noise_var - tmp_1
            if (tmp_1 / tmp_2 > treshold_sinr):
                sum_rate[out_loop][loop] += np.log2(1.0 + treshold_sinr)
            else:
                sum_rate[out_loop][loop]  += np.log2(1.0 + tmp_1 / tmp_2)
    return sum_rate

#CUSTOMIZED SUMRATE_MULTI_LIST_CLIPPED
# Done by Dick Carrillo MElgarejo towards SLicing Scenario
#
def sumrate_multi_list_clipped_interference(H, p, alpha, noise_var):
    # Take pow 2 of abs_H, no need to take it again and again
    H_2 = H ** 2
    N = alpha.shape[0]  # number of links
    M = alpha.shape[1]  # number of channels
...
#Customized SUMRATE_MULTI_WEIGHT_CLIPPED VERSION
#Done by Dick Carrillo because Slicing Scenario
#
def sumrate_multi_weighted_clipped_interference(H,p,alpha,var_noise,weight):
    return sum(np.multiply(weight, sumrate_multi_list_clipped_interference(H,p,alpha,var_noise)))

def sumrate_multi_weighted_clipped_eachslice(H,p,alpha,var_noise,weight):
    sumrate_initial = sumrate_multi_list_clipped_perslicing(H,p,alpha,var_noise)
    sumrate_aux = sumrate_initial
    sumrate_aux = np.multiply(weight, sumrate_initial)
    M_size = np.size(sumrate_initial,0)
    sum_rate_per_slice = [[0.] * M_size for _ in range(1)]
   # print(M_size)
    for ii in range(np.size(sumrate_initial,0)):
        sumrate_aux[ii][:] = np.multiply(weight, sumrate_initial[ii][:])
       # print(ii)
       # print(np.sum(sumrate_aux[ii]))
        sum_rate_per_slice[0][ii] = np.sum(sumrate_aux[ii])
       # print(sum_rate_per_slice[0][ii] )

    #print('hello')
    return sum_rate_per_slice
...
# ADDED BY SLICING SCENARIO - Getting interference
# DONE BY DICK CARRILLO
# Custom Reward_interf_helper()
def reward_interf_helper(H, p, alpha, noise_var, Pmax):
    # Take pow 2 of abs_H, no need to take it again and again
    H_2 = H ** 2
    N = alpha.shape[0]  # number of links
    M = alpha.shape[1]  # number of channels
    ...
    interference = np.zeros((N, N, M))
    for m in range(M):
        for loop in range(N):
            tmp_1 = alpha[loop, m] * H_2[loop, loop, m] * p[loop]
            tmp_2 = np.matmul(H_2[loop, :, m], alpha[:, m] * p) + noise_var - tmp_1

           # sum_rate[loop, :, m] = -((np.log2(1 + np.maximum(0, np.minimum(treshold_sinr, tmp_1 / (
           #             N * [tmp_2] - H_2[loop, :, m] * alpha[:, m] * p))))) - (
           #                              np.log2(1 + np.minimum(treshold_sinr, tmp_1 / tmp_2))))
            interference[loop, loop, m] = tmp_2
           # sum_rate[loop, loop, m] = (np.log2(1 + np.minimum(treshold_sinr, tmp_1 / tmp_2)))
    return interference
...
############### DROP KLL KKTERS    
    ...
    flagCoverage =  mobility_params['fullcoverage']

    #if (N>0):
    for k in range(1):
        if flagCoverage != True:
