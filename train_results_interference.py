...
def main(scenario):    
   ...
   # plt.plot(t, np.array(sum_rate_performance_random)/avg_result_over, label='random',linestyle=next(linecycler))
    ...
    #CUSTOM ADD Slicing
    plt.plot(t, np.array(interference_performance_policy2_db) , label='INTERFERENCE',
    ...
    plt.legend(loc=4)
    plt.tight_layout()
    plt.savefig('./fig/interference%s_network_%d'%(json_file,overal_sims)+'.pdf', format='pdf', dpi=1000)
    plt.savefig('./fig/interference_%s_network_%d'%(json_file,overal_sims)+'.png', format='png', dpi=1000)
  ...
  #  sizeiteration = len(interference_performance_policy2_db)
   ...
   #     for ele in range(sizeiteration):
    ...
    extmatlab.savemat('./fig/interference%s_network_%d'%(json_file,overal_sims)+'.mat', mdict={'time_sample': t, 'interference':interference_performance_policy2_db})
